...
// Signed distance to a tapered capsule-like ribbon with rounded caps.
// Returns: (distToEdge, t01) where t01 is 0 at tail and 1 at head.
vec2 ribbonDistance(vec2 p, vec2 head, vec2 dir, float ribbonLength, float baseWidth) {
  ...
  // Taper: width grows from tail to head using simple lerp
  float halfWidthTail = 0.5 * baseWidth;
  float halfWidthHead = 0.5 * (baseWidth * uHeadMul);
  float halfWidth = mix(halfWidthTail, halfWidthHead, t);
  ...
  float distHeadCap = length(p - head) - halfWidthHead;
  float distTailCap = length(p - tailPos) - halfWidthTail;
...
void main() {
  ...
  float t01 = distAndT.y; // 0 tail → 1 head
  ...
  // Longitudinal fade to zero at tail for comet-like look
  bodyAlpha *= pow(t01, 1.1);
  ...
  glowAlpha *= (0.6 + 0.4 * t01) * sqrt(max(t01, 0.0));
